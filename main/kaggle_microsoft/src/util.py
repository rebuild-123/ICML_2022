...
class ExeDataset(Dataset):
	...
	def __getitem__(self, idx):
		try:
			with open(self.data_path + self.fp_list[idx] + '.txt','rb') as f:
				tmp = [i+1 for i in f.read()[:self.first_n_byte]] # index 0 will be special padding index 每个值加一
				# if self.first_n_byte-len(tmp): print(idx,self.first_n_byte-len(tmp))
				tmp = tmp+[0]*(self.first_n_byte-len(tmp))
		except:
			with open(self.data_path + self.fp_list[idx].lower() + '.txt','rb') as f:
